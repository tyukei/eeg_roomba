...
# main window: 0=orchestrator, 1=gpu-2-chukei, 2=pi-a, 3=pi-b
tmux select-pane -t "$S:main.0" -T orchestrator
tmux select-pane -t "$S:main.1" -T gpu-2-chukei ; tmux send-keys -t "$S:main.1" 'ssh gpu-2-chukei' Enter
...
tmux send-keys -t "$S:logs.0" "ssh gpu-2-chukei 'cd ~/git/eeg_roomba && docker compose logs -f --tail=20 ingest feature decision api'" Enter
...
tmux send-keys -t "$S:logs.3" "ssh gpu-2-chukei 'docker exec eeg_roomba-mosquitto-1 mosquitto_sub -h localhost -t \"#\" -v'" Enter
