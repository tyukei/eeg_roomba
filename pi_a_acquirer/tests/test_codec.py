"""Unit tests for pi_a_acquirer.codec.
...
def test_decode_frame_positive_max() -> None:
    ...
    # raw = 0x7FFFFF = 8388607 (= 2^23 - 1)
    # output = raw * _UV_SCALE = (2^23-1) * 4.5e6 / (6 * (2^23-1)) = 4.5e6 / 6 = 750000 μV
...
def test_decode_frame_negative_max() -> None:
    ...
    out = _decode_frame(bytes(buf))
    # raw = -2^23 = -8388608; output = -8388608 * 4.5e6 / (6 * (2^23-1)) ≈ -750000.09 μV
...
def test_decode_frame_per_channel_independence() -> None:
    ...
    buf = bytearray(_FRAME_BYTES)
    # ch3 occupies bytes 3 + 3*3 = 12, 13, 14
